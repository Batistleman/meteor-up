...
Actions.prototype.deploy = function() {
  ...
  if(this.config.app_git)
  {
    ...
    fs.exists(localFolder, function(exists) {
      if (!exists) {
        git.clone(this.config.app_git.location, localFolder, function(erro, _repo){
          if(!erro){
            console.log("Done cloning.");
            checkoutCommitVersion(_repo);
          } else {
        ...
        checkoutCommitVersion(git(localFolder));
      }
    });
  }
  else{
  ...
  function checkoutCommitVersion(repo){
    console.log("Syncing the repo.");

    repo.sync("origin", self.config.app_git.branch, function(err){
      if(err){
        console.error("An error occured".bold.red);
        console.error(err);
      }else{
        console.log("Checking out " +  self.config.app_git.version);
