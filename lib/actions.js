...
Actions.prototype.deploy = function() {
  ...
  if(this.config.app_git)
  {
    var localFolder = path.resolve("./.app_git_repo/");
    self.config.app = path.resolve(localFolder, self.config.app_git.app_root);

    fs.exists(localFolder, function(exists) {
      if (!exists) {
        process.stdout.write("Cloning repo from " + self.config.app_git.location.blue + " ...");
        git.clone(self.config.app_git.location, localFolder, function(erro, _repo){
          if(!erro){
            console.log("Done cloning.".green);
            syncAndCheckoutTree(_repo);
          } else {
  ...
  function syncAndCheckoutTree(repo){
    process.stdout.write("Syncing the repository to branch " + self.config.app_git.branch.blue +" ... ");
    Hist.bufferObject.branch = self.config.app_git.branch;

    repo.sync(self.config.app_git.branch, function(err){
      if(err){
        console.error("An error occured".bold.red);
        console.error(err);
      }else{

        if(!!self.args[3]) self.config.app_git.version = self.args[3];

        process.stdout.write("and version " +  self.config.app_git.version.blue);
        Hist.bufferObject.version = self.config.app_git.version;

        repo.checkout(self.config.app_git.version, function(){
          repo.current_commit(function(err,commit){
            Hist.bufferObject.commitId = commit.id;
  ...
  function afterCompleted(summeryMap) {
    if(self.config.app_git) Hist.flush();
...
Actions.prototype.version  = function() {
  switch(this.args[3]){
    case "server":
      ...
      break;
    case "config":
      console.log("The Git Version in the config file is            " + this.config.app_git.version.green);
      console.log("The Git Branch in the config file is             " + this.config.app_git.branch.green);
      console.log("The Git remote directory in the config file is   " + this.config.app_git.location.green);
      console.log("The Git root app directory in the config file is " + this.config.app_git.app_root.green);
      break;
    case "history":
      Hist.output();
      break;
